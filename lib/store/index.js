...
export const useAppStore = create(
    persist(
        (set) => ({
            ...
            setMissionStep: (step) => set({ activeMissionStep: step }),
            completeMissionStep: (step) => set((state) => {
                const nextStep = state.activeMissionStep === step ? step + 1 : state.activeMissionStep;
                return { activeMissionStep: nextStep };
            }),
            resetMission: () => set({ activeMissionStep: 0, missionCompleted: false }),
        }),
        {
