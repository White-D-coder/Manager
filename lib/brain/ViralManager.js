...
export class ViralManager {
    ...
    static async executeUploadCycle(contentPacket, accessToken) {
        ...
        // 7.5 REAL VIDEO GENERATION
        console.log("ViralManager: Generating Real Video Asset...");
        // Dynamic import to avoid cycles
        const { VideoRenderer } = await import("./VideoRenderer");
        // Use the first section's visual prompt for the video generation
        const visualPrompt = script.sections[0]?.visual_prompt || script.title;
        const videoUrl = await VideoRenderer.renderVideo(visualPrompt, null); // null for text-to-video
        ...
        const metadata = {
            title: script.title,
            description: "Generated by Social Growth AI.\n\n" + (script.meta?.concept || ""),
            tags: ["#shorts", ...(script.meta?.hashtags || [])],
            publishAt: targetTime
        };

        // Note: YouTubeService.uploadVideo expects a file buffer or stream. 
        // Since we have a URL from Replicate, we need to download it first or pass URL if supported.
        // For this implementation, we assume uploadVideo handles URL or we update it later.
        // Let's pass the URL for now and ensure YouTubeService handles it.
        const uploadResult = await YouTubeService.uploadVideo(videoUrl, metadata, accessToken);
