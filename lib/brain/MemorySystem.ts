export interface SystemMemory {
    ...
    hook_weights: Record<string, number>; // e.g., "shock_value": 1.2
    ctr_baseline: number;
}

const DEFAULT_MEMORY: SystemMemory = {
    total_cycles: 0,
    successful_topics: [],
    blacklisted_topics: [],
    hook_weights: {
...
export class MemorySystem {
    private static KEY = 'growth_machine_memory_v1';

    static load(): SystemMemory {
    ...
    static save(mem: SystemMemory) {
        if (typeof window === 'undefined') return;
        localStorage.setItem(this.KEY, JSON.stringify(mem));
    }

    static recordSuccess(topic: string, ctr: number) {
        const mem = this.load();
        mem.total_cycles += 1;
