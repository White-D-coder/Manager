...
import { TrendItem, GeneratedScript, GrowthConfig } from "@/lib/types";

const API_KEY = process.env.GEMINI_API_KEY;

export class GeminiBrain {
    private static genAI = new GoogleGenerativeAI(API_KEY || "");
    private static model = this.genAI.getGenerativeModel({ model: "gemini-1.5-flash-001" });

    static async analyzeTrends(trends: TrendItem[]): Promise<{ winnerId: string, reasoning: string }> {
    ...
    static async generateScript(trend: String, config: GrowthConfig, tone: string): Promise<any> {
