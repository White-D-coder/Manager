...
`;
  ...
  static async performChannelAudit(stats, videos, webInsights) {
    ...
    // Safety check for videos
    const safeVideos = Array.isArray(videos) ? videos : [];

    // Prepare data for AI (limit to conserve tokens)
    const recentData = safeVideos.slice(0, 15).map(v => ({
      title: v.title,
      views: v.views,
  ...
  static async generateLaunchStrategy(userContext) {
    ...
    const hasInterests = userContext.interests && userContext.interests.length > 2;

    // DYNAMIC PROMPT: If they have no interests, we scan for GLOBAL trends.
    const systemPrompt = hasInterests ? `
      ...
      TASK:
      1. Identify the SINGLE best "Winning Niche" for this user. Be specific.
      2. Explain WHY it works for them.
      3. Create a 4-Phase Roadmap.
      4. Generate 5 "Day 1" video ideas.
    ` : `
      You are a specialized "Zero-to-One" YouTube Launch Strategist.
      
      USER CONTEXT:
      - The user HAS NO IDEA what to create. They want YOU to find the biggest opportunity.
      - Preferred Format: ${userContext.format}
      
      TASK:
      1. Act as a Trend Hunter. Analyze global YouTube trends RIGHT NOW.
      2. Identify the #1 "Blue Ocean" or High-Velocity niche that a new creator should start today (Low competition, High Traffic).
      3. Explain WHY this is the golden opportunity.
      4. Create a 4-Phase Roadmap.
      5. Generate 5 "Day 1" video ideas to capitalize on this trend immediately.
    `;
    ...
    try {
      const result = await this.model.generateContent(systemPrompt + outputInstruction);
      const response = await result.response;
      const text = response.text().replace(/```json/g, '').replace(/```/g, '');
      console.log("DEBUG: Raw Launch Strategy Response:", text);
      return JSON.parse(text);
    } catch (e) {
