...
export class GeminiBrain {
  ...
  static model = this.genAI.getGenerativeModel({ model: "gemini-2.0-flash-exp" });
...
`;
  ...
  static async generateLaunchStrategy(userContext) {
    ...
    const systemPrompt = `
      ...
      TASK:
      1. ANALYZE INTERESTS:
         - If user provided specific interests (e.g. "Minecraft"), find the best high-growth niche within that.
         - If interests are EMPTY or GENERIC (e.g. "idk", "anything"), enter **VIRAL HUNTER MODE**: Ignore their identity and find the #1 Current Highest Velocity Trend on YouTube right now (e.g. "Satisfying Slime", "AI News", "Quiz/Trivia").

      2. Identify the SINGLE best "Winning Niche". Be specific.
      3. Explain WHY it works (either "Matches Interest" or "Global Viral Trend").
      4. Create a 4-Phase Roadmap (Weeks 1-4).
      5. Generate 5 "Day 1" video ideas that are low-effort but high-viral potential.
