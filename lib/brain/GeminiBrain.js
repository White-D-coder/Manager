...
export class GeminiBrain {
  ...
  static fileManager = new GoogleAIFileManager(API_KEY || "");
  static model = this.genAI.getGenerativeModel({ model: "gemini-1.5-flash" }); // Using 1.5 Flash for multimodal speed
...
`;
  ...
  static async optimizeVideoUpload(videoContext, trendContext) {
    ...
    // 1. Handle Video File Upload (if provided)
    if (videoContext.filePath) {
      try {
        console.log("Brain: Uploading video to Gemini...", videoContext.filePath);
        const uploadResponse = await this.fileManager.uploadFile(videoContext.filePath, {
          mimeType: "video/mp4",
          displayName: videoContext.filename,
        });

        fileUri = uploadResponse.file.uri;
        console.log(`Brain: File uploaded ${fileUri}. Waiting for processing...`);
        ...
        while (fileState.state === "PROCESSING") {
          ...
          console.log("Brain: File processing...");
        }
        ...
        console.log("Brain: Video is READY. Analyzing...");
        parts.push({
          fileData: {
    ...
    const systemPrompt = `
      ...
      - Market Context: ${JSON.stringify(trendContext)}
      ${fileUri ? "- VIDEO CONTENT ATTACHED: I have watched the video file." : "- NO VIDEO FILE: Analyzing based on topic only."}

      INSTRUCTIONS:
      1. If a video is attached, analyze the VISUALS and AUDIO. What actually happens? Use that for the description and tags.
      2. If no video, infer from the topic.
